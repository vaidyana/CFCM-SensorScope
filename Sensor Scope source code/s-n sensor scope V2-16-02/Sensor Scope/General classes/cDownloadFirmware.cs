...
namespace Sensor_Scope.General_classes
{
    public class cDownloadFirmware
    {
        ...
        public bool download_file(bool quick,SerialPort serialPort,String sFilename)
        {
            ...
            int tmpBufSize = 128 * 512;
            for (i = 0; i < tmpBufSize - 1; i++) // Force checksum only on first 64k
                global_checksum = global_checksum + hex_buf[i];
            ...
            hex_buf[128 * PAGE_SIZE - 1] = (byte)global_checksum;  // Force global checksum at the end of 64k.
            ...
            for (i = 0; i < PAGES; i++)
            {
//                System.Diagnostics.Debug.WriteLine("Setting page " + i);
            retry:
                if (!set_page(i * 2, 3,serialPort))
                {

                    MessageBox.Show("Download error. Couldn't set page (" + (i * 2) + ").");
...
//                    Debug.WriteLine("Checking page #" + i + "(cfb=" + page_checksum_from_board + " cfm=" + pages_checksum[current_page, 0] + ")");
                    ...
                    {
//                        Debug.WriteLine("Writing page #" + i + "(cfb=" + page_checksum_from_board + " cfm=" + pages_checksum[current_page, 0] + ")");
...
//                    else
//                        Debug.WriteLine("Skipping page #" + i);
                }
                else
                {
                    if (!write_page(current_page,serialPort))
        ...
        private bool set_download_mode(byte mode, SerialPort serialPort)
        {
            ...
            int delay = 0;
            if (mode == CM_DOWNLOADING)
                delay = 3000;
            rc = send_expect(buff, 5, "W", null, 0, timeout_parameter_write,serialPort,delay);
        ...
        private void get_page_checksum(ref long page_checksum, ref long ord_page_checksum, SerialPort serialPort)
        {
            ...
            buff[4] = get_checksum(buff, 4);
            rc = send_expect(buff, 5, "", in_buff, 4, timeout_parameter_read * 20,serialPort);
           // showBuff(in_buff);
            if (!rc)
            {
                page_checksum = ord_page_checksum = -1;
        ...
        void showBuff(byte[] buff)
        {
            Debug.Write("Buff: ");
            for (int i = 0; i < buff.Length; i++)
                Debug.Write("\t" + i + ": " + buff[i]);
            Debug.WriteLine("");
        }
        ...
        private bool send_expect(byte[] send, int send_length, string expect, byte[] response, int response_length, int timeout, SerialPort serialPort,int delay = 0)
        {
            byte[] in_buff;
            int i;
            int local_timeout;
            ...
            try
            {
                if (response_length > 0)
                {
                    ...
                    return (serialPort.Read(response, 0, response_length) == response_length);
                }
