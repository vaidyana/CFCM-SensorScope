...
namespace Sensor_Scope
{
    public class cTesterComm
    {
        ...
        {
            KEY_NOP = 0,
            KEY_AUTO_TEST_START = 1,
            KEY_AUTO_TEST_CANCEL = 2,
        ...
        byte[] testerMem = new byte[MEM_EEPROM_END - MEM_BASE_AAP_EAAP];
        ...
        int[] MEM_SIZES_EEPROM = { 2, 1, 2, 1, 1, 2, 2 ,1,1,1,1,1,1,2,2};
        ...
        const int MEM_EEPROM_END = MEM_BASE_AAP_EAAP + 0X15; // Please update
        #endregion
        ...
        byte[] comCmdReadAllAapPage = { (byte)'R', MEM_BASE_AAP_EAAP >> 8, MEM_BASE_AAP_EAAP & 0xff, MEM_EEPROM_END - MEM_BASE_AAP_EAAP, 0x0 }; // checksum will be calculated on load event.
...
//                MessageBox.Show("Error on tester module: " + e1.Message + "trace: " + e1.StackTrace);
        ...
        void comThread()
        {
            while (true)
            {
                try
                {
                    if (Environment.TickCount - nLastPolling < 50)
                    {
                        Thread.Sleep(0);
                        continue;
                    }
