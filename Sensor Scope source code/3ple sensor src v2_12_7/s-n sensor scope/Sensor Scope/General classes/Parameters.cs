...
namespace PC_Interface
{
    ...
    public class parameter : IComparable
    {
        ...
        public int x_pos = 0;
        public int y_pos = 0;
        public string s_external_change = "";
        public int f_log_changes = -1; // Mark that this field should log changes (-1 not active, >= 1 - column).

        // Fields for the new special logging feature.
        public Boolean sl_do_special_log = false;
        public int sl_comparison_type = -1; 
        public int sl_comparison_value = -1;
        public Boolean sl_mark_when_found = false;
        public double numeric_val = 0;
        public PictureBox sl_marker = null;
...
//                caption.Font = new Font(FontFamily.GenericSansSerif caption.Font, FontStyle.Bold | FontStyle.Underline);
            ...
            try
            {
                org_label = s_label = arr[0].ToString().Trim();
...
//            m_labelToolTip.ShowAlways = true;
        ...
        public void update(byte[] buff)
        {
            ...
            switch (c_type)
            {
                ...
                case 'D':
                    ...
                    Debug.WriteLine("Updating date's text");
                    break;
    ...
    class Parameters
    {
        public String sParamsVersion = "Unknown";
        ...
        public Hashtable hashSpecialParams = new Hashtable();
        public String sManualLogFileName = "";
        private List<String> lstManualFields2Log = new List<string>();
#if (CF)
        ...
        public void setManualLogParams(String file,String fields)
        {
                        // Set manual log vars
            sManualLogFileName = file;
            lstManualFields2Log.Clear();
            String[] sFieldsArr = fields.Split(';');
            Boolean[] ExistingFields = new bool[sFieldsArr.Length];
            Boolean foundMissing = false;
            for (int j = 0; j < sFieldsArr.Length; j++)
            {
                sFieldsArr[j] = sFieldsArr[j].Trim();
                if (sFieldsArr[j] != "")
                    lstManualFields2Log.Add(sFieldsArr[j]);
                foreach (parameter p in params_arr)
                {
                    if (p.org_label == sFieldsArr[j] || sFieldsArr[j] == "")
                    {
                        ExistingFields[j] = true;
                        break;
                    }
                }
                if (!ExistingFields[j])
            ...
            if (foundMissing)
            {
                String sMsg = "The following field(s) doesn't exist:\n";
                for (int i = 0;i<ExistingFields.Length;i++)
                {
                    if (! ExistingFields[i])
                        sMsg += sFieldsArr[i] + ",";
                }
                sMsg += "\n\nMisssing field's value will be blank on the log.";
        ...
        private bool Load_Parameters()
        {
            ...
            try
            {
                ...
                while (!srRead.EndOfStream)
                {
                    ...
                    if (line_count == 1) // Get file's version
                    {
                        lineArray = line.Split(splitChar);
                        sParamsVersion = lineArray[4];
                    }
                    line_count++;
                    ...
                    {
                        if (hashSpecialParams[param.n_special_parameter_code] != null)
                            MessageBox.Show("Special parameter # " + param.n_special_parameter_code.ToString() + " is defined more than once (on " + ((parameter)hashSpecialParams[param.n_special_parameter_code]).s_label + " and " + param.s_label + ").");   
                        else
                            hashSpecialParams.Add(param.n_special_parameter_code,param);
                    }
        ...
        public void ManualSave2Log()
        {
            if (lstManualFields2Log.Count == 0)
            {
                MessageBox.Show("No fields to log.\nPlease use \"Setup==>Set Manual log options\" to update.");
                return;
            }

            if (sManualLogFileName == "" || sManualLogFileName.EndsWith("\\"))
            {
                MessageBox.Show("No filename was defined.\nPlease use \"Setup==>Set Manual log options\" to update.");
                return;
            }
            try
            {
                Boolean fileExist = File.Exists(sManualLogFileName);
                using (StreamWriter sw = new StreamWriter(sManualLogFileName, true))
                {
                    if (!fileExist)
                    {
                        foreach (String s in lstManualFields2Log)
                            sw.Write(s + ",");
                        sw.WriteLine();
                    }
                    ...
                    {
                        Boolean found = false;
                        foreach (parameter p in this.params_arr)
                        {
                            if (p.caption == null)
                                continue;
                            if (p.org_label == s)
            ...
            {
                MessageBox.Show("Writing to log failed: " + e.Message);
            }
        }
    }
